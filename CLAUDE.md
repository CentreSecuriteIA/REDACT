...
- **Constitution-seeded** (`run_from_constitution`, also reached via `generate_inputs(constitution_df=...)`): each constitution entry becomes one batched generation request; one `batch_generate` per chunk for generation, one flat batch for checking. The `fresh` parameter (forwarded from `generate_inputs`) controls resume behaviour: `fresh=False` (default) reads existing per-category CSVs and skips entries whose `(sample_description, entry_type)` is already present; `fresh=True` clears those CSVs first.
