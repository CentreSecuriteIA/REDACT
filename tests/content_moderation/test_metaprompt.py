...
class TestGenerateCategoryDescription:
    def test_returns_description(self):
        ...
        result = generate_category_description(make_client(backend, "model"), "Violence")
        assert "violence" in result.lower()

    def test_retry_on_empty(self):
        backend = MockBackend(["", "   ", "Real description here"])
        result = generate_category_description(make_client(backend, "model"), "Test", max_retries=3)
        assert result == "Real description here"

    def test_fallback_to_category_name(self):
        backend = MockBackend(["", ""])
        result = generate_category_description(make_client(backend, "model"), "Fallback", max_retries=2)
...
class TestGenerateSeeds:
    ...
    def test_fallback_to_empty(self):
        ...
        result = generate_seeds(make_client(backend, "model"), "Test", "desc", max_retries=1
        )
        assert result == ""
